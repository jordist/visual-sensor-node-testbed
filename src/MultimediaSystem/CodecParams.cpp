...
void BRISK_pModel::loadFiles(){
	...
	file.open("util/brisk/P0.bin", ios::in | ios::binary);

	file.read((char*)P0,sizeof(double)*1776);
	file.close();

	file.open("util/brisk/P0c0.bin", ios::in | ios::binary);
	...
	file.open("util/brisk/P0c1.bin", ios::in | ios::binary);
	for(int i=0; i<1776; i++){
		for(int j=0; j<1776; j++){
	...
	if      ( BRISK_LENGTH_BITS == 512){
		file.open("util/brisk/ranking_original_optimized512.bin", ios::in | ios::binary);
	}
	else if ( BRISK_LENGTH_BITS == 256 ){
		file.open("util/brisk/ranking_original_optimized256.bin", ios::in | ios::binary);
	}
	else{
